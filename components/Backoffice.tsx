...
import { 
  ...
  CalendarDays, Zap, Wallet, Key,
  HardDrive, History, CheckCircle2, XCircle, AlertCircle, RefreshCw
} from 'lucide-react';
...
const Backoffice: React.FC<BackofficeProps> = ({ 
    ...
    return new Promise<void>((resolve) => {
      ...
      reader.onloadend = async () => {
        ...
        setNewWorksheet(prev => ({
          ...prev,
          images: prev.images.map(img => img.base64 === resized ? { 
            ...img, 
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setIsUploading(true);
    // Processar ficheiros um a um para manter ordem e estabilidade
  ...
  const handleOpenKeySelector = async () => {
    if ((window as any).aistudio?.openSelectKey) {
      ...
      // Forçar refresh para tentar detetar nova chave
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          <button onClick={onClose} className="text-gray-500 font-bold flex items-center gap-2 mb-2 hover:text-blue-500 transition-colors">
            <ArrowLeft size={18}/> Voltar
          </button>
          <h1 className="text-3xl font-black text-gray-800 flex items-center gap-2">
            <Settings className="text-blue-500" /> Painel de Pais
          </h1>
        </div>
        <div className="flex gap-2">
          <div className="bg-blue-50 px-4 py-2 rounded-2xl border border-blue-100 flex items-center gap-2 text-blue-600 font-black text-[10px] uppercase">
            <HardDrive size={14} /> Memória: {getStorageUsage().percentage}%
          </div>
          {(!process.env.API_KEY) && (
            <button 
              onClick={handleOpenKeySelector}
              className="bg-red-50 px-4 py-2 rounded-2xl border border-red-200 flex items-center gap-2 text-red-600 font-black text-[10px] uppercase hover:bg-red-100 transition-all"
            >
        ...
        {[
          ...
          <button
            ...
            className={`px-6 py-3 rounded-2xl font-bold flex items-center gap-2 transition-all shrink-0 ${
              activeTab === tab.id ? 'bg-blue-500 text-white shadow-lg' : 'bg-white text-gray-500 border border-gray-100 hover:bg-gray-50'
            }`}
          >
      ...
      {activeTab === 'worksheets' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="bg-white p-6 rounded-[35px] shadow-sm border-2 border-blue-50 h-fit space-y-4">
            ...
            <input type="text" placeholder="Nome da Ficha" value={newWorksheet.namePrefix} onChange={e => setNewWorksheet({...newWorksheet, namePrefix: e.target.value})} className="w-full p-3 border-2 rounded-xl focus:border-blue-400 outline-none transition-all" />
            <select value={newWorksheet.subject} onChange={e => setNewWorksheet({...newWorksheet, subject: e.target.value as Subject})} className="w-full p-3 border-2 rounded-xl bg-white outline-none">
              {Object.values(Subject).filter(s => s !== Subject.ALL).map(s => <option key={s} value={s}>{s}</option>)}
            </select>
            
            <div className={`p-6 border-2 border-dashed rounded-2xl text-center transition-colors ${isUploading ? 'bg-gray-50 border-gray-300' : 'bg-gray-50 border-gray-200 hover:bg-blue-50 hover:border-blue-300'}`}>
              <input type="file" multiple accept="image/*" onChange={handleFileUpload} className="hidden" id="ws-file" disabled={isUploading} />
              <label htmlFor="ws-file" className={`cursor-pointer flex flex-col items-center gap-2 ${isUploading ? 'opacity-50 cursor-wait' : ''}`}>
                {isUploading ? <Loader2 className="animate-spin text-blue-500" /> : <Upload className="text-blue-500" />}
                <span className="text-sm font-bold text-gray-500">{isUploading ? 'A processar imagens...' : 'Adicionar Páginas'}</span>
              </label>
            </div>
            ...
            {newWorksheet.images.length > 0 && (
              <div className="space-y-2 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                ...
                {newWorksheet.images.map((img, i) => (
                  <div key={i} className={`p-2 rounded-xl border flex items-center justify-between text-xs animate-in slide-in-from-left-2 ${img.status === 'valid' ? 'bg-green-50 border-green-100' : img.status === 'invalid' ? 'bg-red-50 border-red-100' : 'bg-gray-50'}`}>
                    <div className="flex items-center gap-2 overflow-hidden">
                      <img src={img.base64} className="w-8 h-8 rounded object-cover shrink-0" />
                      <div className="truncate">
                        <p className="font-bold truncate">{img.status === 'validating' ? 'A validar...' : img.status === 'valid' ? 'Pronta!' : 'Erro na foto'}</p>
                        {img.feedback && <p className="text-[9px] text-gray-500 truncate">{img.feedback}</p>}
                      </div>
                    </div>
                    <div className="shrink-0 flex items-center gap-1">
                      {img.errorType === 'api_key' && (
                        <button onClick={handleOpenKeySelector} title="Configurar Chave" className="p-1 text-red-500 hover:bg-red-100 rounded">
                           <Key size={12} />
                        </button>
                      )}
                      ...
                      <button onClick={() => setNewWorksheet(prev => ({...prev, images: prev.images.filter((_, idx) => idx !== i)}))} className="p-1 text-gray-300 hover:text-red-500">
                        <Trash2 size={12} />
                      </button>
                    </div>
                  </div>
            ...
            <button 
              ...
              disabled={!newWorksheet.images.some(img => img.status === 'valid') || isUploading}
              className="w-full bg-blue-500 text-white py-4 rounded-xl font-bold shadow-lg disabled:opacity-50 disabled:bg-gray-300 transition-all active:scale-95"
            >
          ...
          <div className="lg:col-span-2 space-y-3">
            ...
            {worksheets.length === 0 ? (
              ...
              worksheets.map(w => (
                <div key={w.id} className="bg-white p-4 rounded-3xl border flex items-center justify-between shadow-sm hover:border-blue-200 transition-all">
                  ...
                  <button onClick={() => onUpdateWorksheets(worksheets.filter(x => x.id !== w.id))} className="text-red-400 p-2 hover:bg-red-50 rounded-xl transition-colors"><Trash2 size={18} /></button>
                </div>
              ))
            )}
          </div>
      ...
      {/* Outras abas permanecem com a mesma estrutura visual funcional */}
      {activeTab === 'prizes' && (
